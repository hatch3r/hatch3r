...
- **CONSTITUTION codification queued; rules operative now** — recording the model-class allocation and context-budget policies in the CONSTITUTION (a new §6 Decision, a P7 measurement row, D06/D07 checklist rows) is queued for a consent-gated EVOLVE session; the canonical rules ship fully operative without it — codification records the policy, it does not activate it.

...
- **`hatch3r sync` exits non-zero when the workspace cascade leaves any repo failed** — pre-2.8.5, workspace-cascade failures were warn-only: the root summary printed, per-repo errors went to stderr, and the process exited 0 with N of M repos failed, so CI pipelines could not detect the half-state. The cascade now runs BEFORE the summary/JSON emission, the single `--format json` document carries `status: passed | partial | failed` plus a `workspace: { outcome, counts, repos }` payload (per-repo `errorDetail` with `errorCode` / `recoveryHint` / `causeChain` — a `HatchError` is no longer flattened to its message string), and any failed repo ends the run with exit 2 (`ADAPTER_ERROR`), mirroring the existing root-repo partial-failure contract. CI that treated exit 0 as "sync ran" must branch on the exit code (or `.status` in JSON mode); the root-only flow is unchanged.
- **Cross-process write locking is now DEFAULT-ON for every mutating command** — pre-2.8.5 only workspace/worktree commands locked by default and everything else raced last-writer-wins unless the operator discovered `HATCH3R_LOCK=1`; now every write path (init, sync, update, config, mcp, cli-tools, rollback, workspace/worktree) serializes same-file writes across processes via a `proper-lockfile` advisory lock (`<file>.hatch3r.lock` beside each target, ~3s contention budget then a structured `LOCK_TIMEOUT` error). Opt out per run with the new global `hatch3r --no-lock <command>` flag or `HATCH3R_LOCK=0`; `HATCH3R_LOCK=1` force-enables over the flag. Scripts that relied on overlapping unlocked runs now wait (or fail loud with `LOCK_TIMEOUT`) instead of silently clobbering managed files; `enableDefaultCrossProcessLocking()` is retained as a deprecated no-op for one minor release.

### Fixed

- **Published installs no longer resolve an empty content index (BUG-2 / BUG-1)** — `init` and `config` built the content index against the PACKAGE root (`findPackageRoot`), but the published tarball ships canonical content under `dist/content/` only, so every published install silently indexed 0 items: preset rows showed no `(~N items)` counts, `manifest.content.items` was written empty for every preset, and picking Custom crashed with the opaque `[backable-checkbox] No selectable choices` error. Both commands now resolve through `resolveBundledContentRoot()` (the chokepoint the adapter emission path already used); defence-in-depth: `buildContentIndex` fails loud with an actionable FS_ERROR on a zero-item canonical scan, and the Custom picker pre-empts an empty index with a named CONFIG_ERROR instead of the renderer crash. New regression suite pins the `dist/content` published layout (`src/__tests__/content/index.publishedLayout.test.ts`).
- **Tool multi-select renders its pre-selection and never silently substitutes claude (BUG-3)** — the backable checkbox fork declared `default?: V[]` but never read it, so init's and config's tool prompts rendered every row unchecked and Enter-through submitted an empty selection; init then silently rewrote `[]` to `["claude"]`, generating a `.claude/` setup the user never picked. The fork now seeds `checked` from `default` (explicit per-choice `checked` still wins), the tools step also bakes the active default into each choice, and empty-selection semantics are aligned across init and config: `required: true` re-prompts in a TTY, a non-interactive empty resolution throws VALIDATION_ERROR — no silent fallback.
- **Stale adapter outputs are reclaimed when a tool is deselected (BUG-3 secondary)** — a re-init that switched tools (e.g. claude → cursor) left the dropped adapter's files (`CLAUDE.md`, `.claude/`) on disk forever: the fresh manifest discarded the old `managedFilesByAdapter` record and `update`/`sync` deliberately preserve non-run adapters. Init now diffs the previous manifest's recorded adapters against the newly selected tools and sweeps the dropped adapters' recorded paths through the safety-filtered orphan cleaner (adapter-root containment, hatch3r-basename, user-wrapped veto) — with an explicit confirm in interactive mode, `--yes` as consent on headless runs, and skip-plus-warn (never silent deletion) on non-interactive runs without `--yes`; reclaimed files are captured in the pre-mutation rollback snapshot. `config` archives a removed tool's per-adapter record with the files and reclaims stale recorded adapters (entries whose adapter is neither selected nor being removed) behind the same consent.
- **Tracked content selection refreshes on upgrade (BUG-1 root cause)** — `manifest.content.items` was written once at init and never re-resolved, so a repo initialized at an older hatch3r carried its era's item lists forever (artifacts shipped by later versions, e.g. the qa-path cluster, never appeared in `status`/`explain`/sync summaries). A new `content-selection-refresh` migration checkpoint in `hatch3r update` re-resolves the selection against the freshly bundled corpus whenever the manifest's recorded `hatch3rVersion` differs from the running one — preset/projectType/teamSize preserved verbatim, the persisted role/facets filter and stored language set re-applied, and `custom` honouring the stored id list (new floor/protected artifacts admitted additively; user-removed optional ids never resurrected).
- **init/config parity: the config features checkbox is gone (BUG-4)** — init never prompted for features, and config's checkbox let an accept-defaults run silently rewrite live feature flags. Feature flags now derive from the persisted manifest (schema defaults fill keys a legacy manifest omits — surfaced as an explicit `Features enabled` diff line once); the MCP gate keys on the persisted `features.mcp` (opt-in stays behind `hatch3r mcp setup` / `init --mcp`). Same pass: config's selection re-resolution now threads the stored language set and runs the same context filters init runs (no more phantom "Content added" items on an accept-defaults run), and the `--role`/`--facets` filter chosen at init is persisted under the new additive `manifest.contentFilter` field and honoured by config and the update-time refresh instead of being silently dropped.

### CLI

- **Communication style graduates to an interactive prompt** — the 2.8.0 flag-only `communicationStyle` dial is now prompted: unconditionally in `hatch3r config` (after the confidence floor) and conditionally in `hatch3r init` (skipped when `--communication-style`, a headless flag, or `--resume` resolves it). Init's interactive max path is now 7 prompts — one over the Decision 25 ≤6 ceiling; the governance amendment raising the ceiling is queued for the next EVOLVE session and the overage is recorded honestly in the code. Flag-driven runs stay ≤6.

### Content

- **Browser verification redesigned around a three-tier invocation contract** (`rules/hatch3r-browser-verification.md`, `skills/hatch3r-browser-verify/SKILL.md`, implementer §5b, fixer, reviewer, 5 command opt-in blocks → one shared block in `commands/shared/orchestration-frame.md`) — Tier 1 default: write/update a durable Playwright spec and run `npx playwright test <spec> --reporter=line` headless, reading only failures (visual checks stay in the runner as `toHaveScreenshot()` diff counts); Tier 2: step-driving only for exploration, via `playwright-cli`/accessibility snapshots, never screenshots for state-reading; Tier 3: screenshots only for inherently-visual checks. The `N/A (no browser MCP available)` free-pass token is replaced corpus-wide by the non-launderable set `VERIFIED-SPEC | VERIFIED-INTERACTIVE | BLOCKED_MISSING_TOOL | N/A-NO-UI`, `tool-not-configured:` annotations must name the missing tool + install one-liner and downgrade the surface to `blocked`, and all browser verification runs sub-agent-isolated with a per-adapter dispatch table (Claude Code / Cursor / Copilot). Grounded in 19-source research (Playwright CLI/MCP/Test-Agents official docs + independent token measurements, accessed 2026-07-28).
- **Review-loop convergence policy kills the endless-re-review failure mode** (`rules/hatch3r-agent-orchestration.md` + detail rule, `agents/hatch3r-reviewer.md`, fixer, `agents/shared/confidence-gate.md`, review-loop-cap hook, all 10 cap-bearing commands) — the cap scheme is stated once (loop-class caps 3 code / 4 spec under ceiling `DEFAULT_MAX_REVIEW_ITERATIONS`, overrides clamped to `HARD_MAX=10`); from iteration 2 re-review is delta-scoped to the fixer's changed hunks + verify-fix findings with the 20-surface checklist, consumer census, grounding re-execution, and external verification signals all iteration-conditional; findings are fingerprinted (file + normalized hunk + class) and dispositioned fingerprints on unchanged code are never re-raised; iterations ≥2 re-open only Critical/Warning; cap-out is an UNRESOLVED escalation (the confidence-gate second-full-pass-at-cap amplifier is removed) — never a silent continuation.
- **qa-path rows are proven before the human sees them** (`skills/hatch3r-qa-path/SKILL.md` Step 3.5) — per-row proving delegated to `hatch3r-qa-validation` (functional/E2E) and `hatch3r-browser-verify` (UI) via the Task tool; the schema gains `Proven (auto)` + `proof_trace` columns and the emitted manual table lists only unproven/partially-proven rows (proven rows move to an appendix with traces). The reviewer now hands off to it: at a loop-exit verdict on a diff with user-observable behavior change, `agents/hatch3r-reviewer.md` → QA-Path Handoff runs the skill once (never per iteration) and reports `QA path: emitted (<n> unproven rows)` in the review summary.
- **Website security-audit CI gate scoped to actionable advisories** (`.github/workflows/ci.yml` + new `website/scripts/audit-gate.mjs`) — the bare `--audit-level=high` step was permanently red on the unfixable `serve-handler`→`minimatch`→`brace-expansion` chain; the gate now allowlists exactly that root advisory id (1124334 / GHSA-mh99-v99m-4gvg), fails on any other high/critical advisory including new ones in the same subtree, and prints a removal notice once the upstream fix lands.
- **New `hatch3r-cleanup` skill (55th)** — repo-state cleanup pass over accumulated agentic working state: inventories plan files (`docs/rework/`, `docs/specs/` orphans), workspace dirs, review telemetry, findings ledgers, learnings, handoffs, and board staleness; one consolidated ASK with per-category counts; dispatches owned surfaces to their owning skills (learn consolidation, board-groom archive, sync `pruneHandoffs`, ledger prune bounds) and prunes only the unowned remainder — never touches `.hatch3r/overrides/`, never deletes learnings.
- **New `hatch3r-spec-currency` rule (74th)** — any diff changing user-observable behavior covered by a `docs/specs/` file updates that spec in the same delivery (implementer proposes, docs-writer applies); reviewer emits an owner-named spec-drift finding; release-prep sweeps spec currency; wired into implementer (`Spec updated:` result field), docs-writer (binding gate), product-spec (per-feature fidelity at all tiers, amendment authority resolved), and unconditional docs-writer Phase-4 dispatch on covered behavior change.
- **Dependency-CVE checks reach every security surface** — `agents/hatch3r-security.md` verification table gains polyglot rows (`npm audit --audit-level=high`, `osv-scanner -r .`) beyond the GitHub-only Dependabot query; `github-agents/hatch3r-security-agent.md` gains a blocking dependency-CVE scan step; `commands/hatch3r-security-audit.md` gains sub-issue 5c (scan → per-advisory research → mitigated/accepted verdict with evidence URL); `checks/security.md` carries the ≤90-day staleness window + OSV/GHSA sources; the reviewer gains a `deps.review` row (new/raised advisory on an added-or-bumped dependency = Critical).
- **CLI-toolbox becomes reachable** — `rules/hatch3r-tooling-hierarchy.md` gains an Installed CLI Tools tier (the always-on five + the `skills/hatch3r-cli-toolbox` registry incl. `playwright-cli`) between platform tools and web research; the toolbox skill documents its invokers.

### Dependencies

- Folded all 9 open dependabot PRs (#113, #120, #127, #128, #138, #139, #140, #143, #144): root prod `ora` 9.4.1 + `p-limit` 7.3.1; root dev `@types/node` 26.1.1, `@vitest/coverage-v8` 4.1.10, `eslint` 10.7.0, `tsx` 4.23.1, `typescript-eslint` 8.65.0; website `@docusaurus/*` 3.10.2, `typescript` 7.0.2, `react`/`react-dom` 19.2.8; GitHub Actions SHA bumps `harden-runner` v2.20.0, `checkout` v7.0.1, `setup-node` v7.0.0, `gh-aw/setup` 0.81.6, `action-gh-release` v3.0.2.
- Cleared every root npm-audit advisory with an available fix beyond the PRs: `brace-expansion` 5.0.8 (CVE-2026-14257, high) and `postcss` ≥8.5.18 (GHSA-r28c-9q8g-f849, high, via #128); website lockfile fixes for `fast-uri` 3.1.4 (2 CVEs) and `dompurify` 3.4.12. Residuals documented, gates untouched: root `esbuild` 1 low (tsup still pins `^0.27.0`; excluded from CI gate shapes), website `serve-handler`→`minimatch`→`brace-expansion` chain (no upstream fix exists — the website security-audit CI step stays red) and sockjs-nested `uuid` (unfixable without a sockjs release).
- CI: build matrix gains a pinned Node `22.0.0` floor lane (ubuntu) so the `engines >=22.0.0` contract is actually tested; website editor tsconfig inlined from `@docusaurus/tsconfig` minus the TS7-removed `baseUrl`.

### Internal

- Config-ingress hardening: new `src/config/parse.ts` shared parse primitives; workspace-manifest migrations registry + major-version gate + per-field error accumulation + normalized duplicate-repo-path rejection + unknown-field advisories; validated MCP-config documents; pack-manifest whitelist construction (third-party supply); snapshot-meta validation; customize unknown-field warnings.
- Import boundaries enforced: ESLint `no-restricted-imports` (domain modules must not import `src/cli/**` or CLI/UI libraries) + an architecture test; `generateBridgeOrchestration` moved out of the CLI layer into `src/adapters/shared/`; the `cliTools/install.ts` inquirer prompt hoisted to its CLI caller.
- Characterization suites for workspace sync (resolved-plan, write-ordering, mixed-outcome partial failures) and CLI JSON envelope key-set stability + command-level lock outcomes; `src/workspace/**` coverage floor pinned at measured 85/74/95/87.

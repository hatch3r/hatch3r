...
2. If `.hatch3r/learnings/` exists, scan for relevant learnings — test the affected file paths against each learning's `applies-to` glob and the affected area against its `topic` (canonical match keys per `rules/hatch3r-learning-system.md`; accept legacy `area`/`tags` only as a transitional fallback). Learnings are **consulted only** in this command — debug fixes the immediate defect and does not write new learnings (deliberate skip, parallel to `hatch3r-quick-change` Token-Saving Directive 4); durable bug-pattern learnings are captured by `hatch3r-workflow` Phase 4e when a fix lands through that flow and by `hatch3r-rework` Step 10 during rework-feedback triage. Iteration-summary item 9 ("Learnings Captured") therefore reads `None` for a standalone debug run.

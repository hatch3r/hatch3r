...
- **Never commit or push; the run ends at the rework plan plus the Execute-or-Defer choice.** On defer, the copy-paste execution prompt closes the run; on execute now, execution proceeds in-session under the workflow command file's own pipeline, which owns commit semantics — this command itself still never commits or pushes. The plan document stays uncommitted in the working tree — the execution pass (in-session or fresh-session) commits it together with the fixes. No `git add`, `git commit`, or `git push` from this command in any mode.
...
- **Read-only sub-agents only (planning pass).** This command composes `hatch3r-researcher` and `hatch3r-reviewer` — no implementer, lint-fixer, or fixer spawn. A spawn of any code-mutating agent is out of contract during the planning pass; after the Execute-or-Defer ASK selects execute now, execution runs under the workflow command file's pipeline, not this command's contract.

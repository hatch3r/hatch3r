...
Auto-tiering can misclassify — a clear single-module bug scored as Deep, or a multi-module incident scored as Light. The user override is the recovery path mandated by hatch3r's universal `--effort` override contract ("User overridable via `--effort` flag"):

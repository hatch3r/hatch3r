...
This wordlist mirrors the single physical home `governance/CONSTITUTION.md` §2 P5 → Anti-Slop Wordlist row-for-row, as does `CLAUDE.md` → Anti-Slop Wordlist — those two are the only row-for-row mirrors, refreshed in the same sweep. Three further surfaces derive from the same home WITHOUT matching it row-for-row, each also refreshed in the same sweep (D24-SA24.1-03, D5-SA5.3-07): `governance/AUDIT-EXECUTE.md` regression gate 11 is a one-way pointer that holds no rows (it does not re-define the list); `scripts/validate-anti-slop.ts` is a scoped marketing-surface subset (README/SECURITY/CONTRIBUTING/current-CHANGELOG/package+plugin descriptions) that omits the prose-context rows and adds marketing-only patterns — its file header defines its pattern set; and `src/content/userContent.ts::ANTI_SLOP_WORDLIST` is the runtime user-content gentle gate (D20) that mirrors the wordlist for user-authored artifacts. A hit without the paired measurable qualifier within 8 words is a gate failure.

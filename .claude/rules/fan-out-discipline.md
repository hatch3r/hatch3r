...
Omitting the field — or its required task-structure companion — on a delegating artifact is a P8 violation.
...
Evidence-class ranking: an attestation cites the highest evidence class available — (1) an infrastructure-recorded invocation/trace outranks (2) the sub-agent's structured-result file on disk, which outranks (3) a self-quoted token (fallback-only). The verbatim `delegation_proof_id` quote is class 3; when the sub-agent wrote a results file or the platform recorded the Task invocation, cite that stronger class.

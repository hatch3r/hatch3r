...
Why it is forgery-resistant: the per-file `delegation_proof_id` is returned by `hatch3r-implementer` in its structured result (`agents/hatch3r-implementer.md` -> Return Structured Result). The orchestrator quotes it verbatim. An orchestrator that skipped delegation has no `delegation_proof_id` to quote — its row is unattributable, which is a self-declared P8 B2 violation per the block's rules.

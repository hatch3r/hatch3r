...
12. **Tag ordering — primary classification first (D10-SA10.6-F10.6-8):** the FIRST tag in an artifact's `tags:` array is its primary classification. The custom-content picker (`src/cli/shared/customContentChoices.ts`) groups each artifact under `tags[0]`, so order is load-bearing for discoverability, not cosmetic. Put a **capability tag** (`orchestration`, `planning`, `implementation`, `review`, `devops`, `maintenance`, `board`, `performance`, `ai`) first; never lead with a context tag (`ctx:*` such as `ctx:team-only`, `ctx:brownfield-only`) or a `floor:*` tag — those belong after the primary capability. Example: `tags: [implementation, review, ctx:team-only]` groups under Implementation (correct); `tags: [ctx:team-only, implementation]` would mis-group it under a context bucket. An artifact whose only tags are context/floor tags has no capability primary — give it one or accept the `Other` group.

...
13. **Citation-path policy — runtime bodies never cite `.claude/rules/*` or `governance/*` (D5-9, extended D5-SA5.2-09):** canonical artifacts under `agents/`, `skills/`, `rules/`, `commands/`, `hooks/` SHIP to end-user repos verbatim via `src/adapters/canonical.ts::readCanonicalFiles` (`agents`, `skills`, `rules`, `commands`, `hooks` are the ship-verbatim subset of its `CanonicalType` discriminant; `.claude/rules/` is framework-dev-only and is NOT a canonical dir, so a `.claude/rules/*` substring inside a shipped body is a dangling reference in the user's repo). **`governance/*` is banned identically:** `governance/` was privatized 2026-06-03 — only `governance/inventory.json` is tracked in the public repo and `scripts/copy-content.ts` SOURCE_DIRS never bundles `governance/`, so a `governance/<path>` citation inside a shipped runtime body dangles for every consumer outside the maintainer overlay. `governance/` cites have no shipped twins, so they always take the no-twin branch below — inline the criterion (e.g. inline a pack trust-tier ladder + lifecycle-script ban rather than citing `governance/pack-trust-model.md` §1/§4). Rule for a runtime body that needs to cite a `.claude/rules/<name>.md` rule:
    - **Has a canonical runtime twin** → cite the twin path. Current twins: `clarification-default` → `rules/hatch3r-clarification-default.md`; `security-patterns` → `rules/hatch3r-security-patterns.md`; `fan-out-discipline` → `rules/hatch3r-fan-out-discipline.md`. Twin substitution presumes the cited concept exists in the twin; the security-patterns pair corresponds by name/role, not body (the dev rule covers hatch3r `src/` internals, the twin covers the end-user OWASP surface) — for framework-internal patterns use the no-twin branch below.
...
| Cursor **Manual** mode (`@`-mention only) | not emitted (no transform target) | **unsupported** — the generated corpus never emits Cursor's Manual / `@`-mention rule shape; a rule that should load only on explicit mention is authored `scope: agent-requested` (description-driven pull) instead, so authors do not improvise a Manual variant (D5-SA5.4-10) |
...
The `.mdc` twin inherits `precedence` unchanged from the `.md` source. `scripts/validate-rule-parity.ts` (CI gate `npm run validate:rule-parity`) verifies `.md`/`.mdc` precedence parity across all rule pairs. The framework-dev gatekeeper enumeration in the `high`-precedence row above is an informational prose list, hand-maintained — it is not derived from or machine-cross-checked against the on-disk `precedence:` frontmatter. D05 audits review assignment-policy compliance per cycle by reading the rule frontmatter directly. For `.claude/rules/*` files the `precedence:` frontmatter key is nominal — these files are never adapter-emitted, so no `NN-` prefix or ordering applies; the assignment selects the lean-cap class only (`high` → ≤250 lines, `normal`/`low` → ≤120).

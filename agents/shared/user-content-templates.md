...
Four scope shapes (pick one):

| Shape | `scope:` line | `globs:` line | When to use |
|---|---|---|---|
| A | `scope: always` | (omit) | Applies every session, regardless of file context. |
| B | `scope: "<GLOB-CSV>"` | (omit) | **deprecated** — do not author (use Shape C). Legacy inline-CSV form: still round-trips to a valid `.mdc`, but new rules must not emit it. |
| C | `scope: conditional` | `globs: "<GLOB-CSV>"` | Explicit globs plus optional `precedence:`. |
| D | `scope: agent-requested` | (omit) | Cursor Apply-Intelligently: description-only (`.mdc` gets `alwaysApply: false`, no globs). For a large optional non-floor rule with no natural glob — stays out of Cursor's always-on budget until the agent pulls it in. |
...
type: rule
description: <DESCRIPTION>
scope: <always-OR-agent-requested-OR-conditional>
globs: "<GLOB-CSV>"          # required for Shape C; omit for A/B/D

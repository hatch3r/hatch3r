...
This protocol defines how hatch3r agents and commands surface clarifying or triage questions across the 4 supported AI coding tools (Claude Code, Cursor, GitHub Copilot, and skills-only Codex). It is the single source of truth for the *how* of asking; the *whether* is governed by [quality-charter §3 "Question Unclear Requirements"](./quality-charter.md) and §8 "Escalate Ambiguity Early". Coverage is a 100% floor, not a fixed file list: every framework-dev workflow that can mutate canonical artifacts routes its ASK through this protocol — the requirements-elicitation mode (`agents/modes/requirements-elicitation.md`), the shared §0 gate block (`agents/shared/clarification-default-block.md`), and every `agents/hatch3r-*.md` agent and `commands/hatch3r-*.md` command that detects ambiguity (counts: `governance/inventory.json` `counts.agents`, `counts.commands`, `counts.skills`). The supported-tools figure above is drift-guarded against `inventory.json` `counts.adapters` by `scripts/inventory.ts` (`npm run inventory:check-docs`).
...
**Author invariant — this marker is safe only inside companion-subdir files.** Marker substitution runs only on emission paths that call `substituteCanonicalContent` (`src/adapters/base.ts`), which composes `substituteAskUserMarker` (the marker's replacer) ahead of the repo-token pass: that path covers the companion subdirs (emitted via `processCompanionSubdir`, this file's own route) and Copilot's per-file rule/agent bodies. Cursor and Claude instead emit top-level `agents/*.md` and `rules/*.md` bodies through the token-only `substituteDetectedRepoTokens`, which never replaces this marker — so a marker placed in a top-level agent or rule body would ship verbatim into `.cursor/` and `.claude/` output while Copilot substituted it. Keep `<!-- HATCH3R:PLATFORM-TOOL -->` inside companion-subdir files (like this one) only.
...
1. **Detect non-response.** If a question goes unanswered within the host runtime's question window — Claude Code injects a synthetic user `tool_result` with empty `answers: {}` plus "…Proceed using your best judgment based on the context so far" after `CLAUDE_AFK_TIMEOUT_MS` (AFK auto-continue, default 60000ms; no first-class opt-out — `=0` fires immediately, only a large integer suppresses it); Cursor: AskUserQuestion timeout; Copilot Workspace: prompt-cycle gap — or if the user replies "you decide" / "default" / empty, treat as non-response.
...
**AFK reconciliation (Claude Code).** When AFK auto-continue fires, its empty-`answers` payload IS the step-1 non-response signal: apply the declared `Default if no response` option (step 2) and emit the `Default applied:` line (step 3). The injected "proceed using your best judgment" text does NOT license an undeclared silent-pick — a default was named, so step 2 governs over the harness suggestion; only a genuinely missing default line routes to `BLOCKED_AMBIGUITY` (step 4). Re-verify this payload each cycle on the same D09 web-research pass that stamps `src/pipeline/adapterToolTranslator.ts::ASK_USER_TOOLS` (`anthropics/claude-code` #73408, accessed 2026-07-11).

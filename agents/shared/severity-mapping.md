...
2. **CQ pillar regression rule.** A finding that introduces a measurable regression on a CQ1-CQ10 pillar threshold (per the content-quality (CQ) pillars and their specialist agents) escalates one canonical band regardless of source bucket. The CQ specialist's `Specialist Status` column already collapses to PASS | FINDINGS | CRITICAL — the escalation re-expands to the canonical band the regression actually represents per the regression evidence file path + line cited in the finding body.

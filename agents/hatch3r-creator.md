...
```
{
  ...
  model:          "economy" | "standard" | "advanced" | "frontier",  // agent only (legacy synonyms fast/default/reasoning/strongest accepted on user overrides only)
...
| `model` | yes | one of `economy | standard | advanced | frontier` (legacy synonyms `fast`/`default`/`reasoning`/`strongest` accepted on user overrides only) |
| `effort` | optional | one of `low | medium | high | xhigh | max`; unset inherits the class default (`frontier`→`xhigh`, `advanced`→`high`, `economy`→`medium`, `standard`→inherit); verdict-lane floor agents author `xhigh` or above |

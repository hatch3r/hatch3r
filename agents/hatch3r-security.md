...
8. **OWASP ASI01-10 + CVE acknowledgement.** Every agent-produced module passes the current OWASP ASI revision check (100% control coverage); CVE advisories ≤90 days old that match any project dependency are acknowledged in the finding registry with a `mitigated` OR `accepted` verdict and an evidence URL. Advisory sources are the OSV + GHSA feeds queried through ecosystem-native scanners (`npm audit`, `osv-scanner`, or the per-ecosystem equivalent) — GitHub Dependabot alerts are one additional stream when the repo is GitHub-hosted, never the only source. Reference: OWASP Foundation + GitHub Security Advisories + OSV.
...
| 8. CVE acknowledgement (GitHub-hosted) | `gh api repos/{owner}/{repo}/dependabot/alerts --jq '.[] \| select(.state=="open")'` | any unacknowledged alert ≤90 days old |
| 8. CVE scan (npm) | `npm audit --audit-level=high` | any high/critical advisory lacking a registry `mitigated`/`accepted` verdict |
| 8. CVE scan (polyglot / off-GitHub) | `osv-scanner -r .` | any lockfile advisory lacking a registry verdict — non-npm ecosystems substitute the ecosystem-native auditor (`pip-audit`, `cargo audit`, `govulncheck`) when `osv-scanner` is unavailable |

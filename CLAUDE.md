...
| `agents/` `skills/` `rules/` `commands/` `hooks/` | Canonical content artifacts. Agent surface: 16 main + 20 modes (`agents/modes/`) + 3 shared (`agents/shared/`) = 39 files total; `inventory.json` counts only the 16 main agents. Other artifacts: 26 skills, 27 rules, 34 commands, 6 hooks. Authoritative counts: `governance/inventory.json` (regenerated by `npm run inventory`, drift checked in CI) |

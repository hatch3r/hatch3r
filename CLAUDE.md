...
| `agents/` `skills/` `rules/` `commands/` `hooks/` | Canonical content artifacts. Agent surface: 17 main + 20 modes (`agents/modes/`) + 3 shared (`agents/shared/`) = 40 files total; `inventory.json` counts only the 17 main agents. Other artifacts: 26 skills, 28 rules, 37 commands, 6 hooks. Authoritative counts: `governance/inventory.json` (regenerated by `npm run inventory`, drift checked in CI) |
| `checks/` `prompts/` `github-agents/` | Additional content: 6 checks, 3 prompts, 4 github-agents |
| `governance/` | 7 Binding Pillars, 21 audit domains, audit cycle, closed-loop phases |

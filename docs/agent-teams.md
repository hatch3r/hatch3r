...
Hatch3r agent definitions in the bundled `agents/` directory (emitted by the Claude adapter to `.claude/agents/hatch3r-*.md`) map directly to teammate spawn prompts. Each agent's `description` and content become the teammate's role instructions.

| Hatch3r Concept | Agent Teams Equivalent |
|-----------------|----------------------|
| Agent definition (`.claude/agents/hatch3r-*.md`) | Teammate spawn prompt |
...
| Rule (`.claude/rules/hatch3r-*.md`) | Included in teammate instructions or referenced |
...
2. **Reference skills explicitly** — tell the teammate to read specific `SKILL.md` files: `"Read .claude/skills/hatch3r-issue-workflow/SKILL.md for your workflow."`.
...
- **Project conventions** — reference `CLAUDE.md` and specific `.claude/rules/` files so teammates follow project standards.

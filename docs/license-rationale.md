...
The maintainer's intent is captured in [`governance/VISION.md`](../governance/VISION.md) §Distribution: "hatch3r is open-source. The focus is on building the ideal framework first. Distribution channels (npm, marketplace plugins, private registries) are secondary concerns that follow from getting the framework right."
...
- [`governance/VISION.md`](../governance/VISION.md) §Distribution — vision-level distribution stance

...
**Emission-template limitation (D11-SA11.4-01).** The hook → adapter emission template models a single archetype — "on event Y, spawn agent X" — and interpolates this hook's `agent: reviewer` binding into that spawn directive on every adapter (Claude `.claude/settings.json`, Cursor `.mdc`). This hook is the second archetype — a gate that BLOCKS an action — which the template cannot represent, so the generated guidance reads "spawn the reviewer agent" (the inverse of this hook's cap-enforcement contract), and on Claude the `Stop` mapping echoes it at every turn end. The `agent: reviewer` value is a template-satisfying placeholder, not a spawn intent: no `review-loop-cap-enforcer` agent exists. The systemic fix is a hook `action: spawn | gate` discriminant on `HookDefinition` (`src/hooks/types.ts`) plus a per-kind emission branch in `src/adapters/claude.ts` and `src/adapters/cursor.ts` so a `gate` hook renders a block directive ("on event X, block the fixer-spawn past the cap — do NOT spawn an agent"). Until that lands, treat the emitted "spawn the reviewer agent" text for this hook as advisory only and do NOT re-enter review on it. Cross-domain: D5/D22 (hook authoring) + D9 (adapter hook emission).

...
This hook specifies the target contract for the F15.2-H1 gap surfaced in cycle 10: the runtime `src/pipeline/reviewLoop.ts` carries the operative loop-class caps (`REVIEW_LOOP_CLASS_CAPS` — 3 code-diff / 4 spec-text) under the protocol ceiling `DEFAULT_MAX_REVIEW_ITERATIONS = 4` and the override clamp `HARD_MAX_REVIEW_ITERATIONS = 10` (unified cap scheme: `rules/hatch3r-agent-orchestration.md` Phase 3), but those bounds are enforced only inside the reviewer/fixer agent prompts, not at the adapter-emitted hook layer.
...
- **Cursor:** no counter-aware runtime gate — advisory rule only (parity with GitHub Copilot below); a coarse `subagentStop` `loop_limit` backstop exists but does not read the per-issue counter. The iteration-count gate needs the orchestrator's per-issue `.review-loop.json` counter context, which no Cursor hook payload carries; Cursor's real pre-tool event is `preToolUse` (NOT `pre-tool-call`), and its payload exposes no agent-identity field (cursor.com/docs/agent/hooks, accessed 2026-06-09), so it cannot bind to a fixer-spawn. Cursor's current hooks doc (cursor.com/docs/hooks, accessed 2026-07-10) documents a per-script `loop_limit` field (default 5) on the `stop`/`subagentStop` events that bounds hook-forced continuations — a coarse spawn-count ceiling, not a `.review-loop.json`-aware gate — so the earlier absolute "no native runtime gate" wording is re-graded (D9-SA9.2-02, Cycle 12) to "no counter-aware gate; a coarse `subagentStop` `loop_limit` backstop exists." Materialized as the `.cursor/rules/hatch3r-hook-hatch3r-review-loop-cap.mdc` advisory rule the Cursor adapter emits for every canonical hook.
...
- **maxIterations:** Default = the active loop-class cap from `src/pipeline/reviewLoop.ts::REVIEW_LOOP_CLASS_CAPS` — 3 for code-diff loops, 4 for spec/issue-text loops; when the orchestrator supplies no loop class, fall back to `DEFAULT_MAX_REVIEW_ITERATIONS = 4` (the protocol ceiling). Same unified scheme as `rules/hatch3r-agent-orchestration.md` Phase 3 — this hook holds no private number. Override via `maxIterations` in hook config. Clamped to `[MIN_MAX_REVIEW_ITERATIONS, HARD_MAX_REVIEW_ITERATIONS]` = `[1, 10]` per the same module.
...
**Emission template — gate statement (D11-SA11.4-01).** The hook → adapter emission template models a single archetype — "on event Y, spawn agent X" — and interpolates this hook's `agent: reviewer` binding into that spawn directive on every adapter (Claude `.claude/settings.json`, Cursor `.mdc`). This hook is the second archetype — a gate that BLOCKS an action — so the template's generated "spawn the reviewer agent" line inverts the contract; the `agent: reviewer` value is a template-satisfying placeholder, not a spawn intent (no `review-loop-cap-enforcer` agent exists). **The gate this hook states — binding on every adapter, overriding any generated spawn line:** at the fixer-spawn boundary, when the incremented `iteration` exceeds `maxIterations`, BLOCK the fixer-spawn, emit the structured block payload, and escalate ("findings remain after N iterations — maintainer review or accept current state"); spawn no agent and do not re-enter review. The systemic emission fix is a hook `action: spawn | gate` discriminant on `HookDefinition` (`src/hooks/types.ts`) plus a per-kind emission branch in `src/adapters/claude.ts` and `src/adapters/cursor.ts` so a `gate` hook renders the block directive natively; until that lands, this paragraph — not the generated spawn text — is the hook's operative contract on every adapter. Cross-domain: D5/D22 (hook authoring) + D9 (adapter hook emission).

...
Native primitive: on the Vercel AI SDK (6+, retained in 7), this gate maps to the built-in `needsApproval` flag on a tool definition (boolean, or a function of the tool input for conditional gating) — the SDK pauses the tool call with no hand-rolled pause/resume state: the tool part enters `approval-requested`, the client answers via `addToolApprovalResponse({ id, approved })`, and the part moves through `approval-responded` to `output-available` or `output-denied` (`ai-sdk.dev/docs/ai-sdk-ui/chatbot-tool-usage`, accessed 2026-07-12). Wire the approval card to that primitive on AI SDK stacks; hand-roll the gate only on stacks without an equivalent.
...
- Vercel AI SDK UI docs (`ai-sdk.dev/docs/ai-sdk-ui`; tool parts + approval flow: `ai-sdk.dev/docs/ai-sdk-ui/chatbot-tool-usage`; accessed 2026-07-12) — `useChat`, `useCompletion`, `useObject`
- Vercel — AI SDK 6 (`vercel.com/blog/ai-sdk-6`, GA 2025-12-22) and AI SDK 7 (`vercel.com/blog/ai-sdk-7`, current stable major, GA 2026-06-25) — `message.parts` model, `sendMessage`, `needsApproval` human-in-the-loop, `Agent`/`ToolLoopAgent` (accessed 2026-07-12)
- Vercel AI SDK RSC — `streamUI()` (`ai-sdk.dev/docs/ai-sdk-rsc`; experimental — the docs recommend AI SDK UI for production; accessed 2026-07-12)

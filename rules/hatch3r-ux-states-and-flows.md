...
### Persisted flow-doc convention

Each decomposed flow persists to `docs/flows/<flow-name>.flow.md` — one file per user story, kebab-case slug (e.g. `docs/flows/sign-up.flow.md`). Every decision point (a branch offering 2 or more user-selectable options) is recorded on its own line beginning `- decision: <one-line label>`; this line is the machine-countable serialization of the `### Decision Points` rows in the `agents/modes/user-flows.md` template. The decisions-per-flow budget of at most 3 is verified with `rg -c "^- decision:" docs/flows/<flow-name>.flow.md`; a count above 3 fails the budget and triggers a reduction proposal (smart default plus override flag), not a tooltip patch.

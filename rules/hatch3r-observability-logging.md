...
These rules split observability by signal for separate maintenance, not to load a single signal per edit on backend paths: this rule and `hatch3r-observability-metrics` share a backend-path glob (`observability/`, `routes/`, `handlers/`, `services/`, `api/`, `middleware/`, `controllers/`, `lib/`), so an edit under those paths activates both — a service handler there emits logs and metrics together. `hatch3r-observability-tracing` is `agent-requested`, so it loads on demand rather than by path.

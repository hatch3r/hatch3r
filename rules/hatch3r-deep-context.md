...
|-------------|------|-------|---------------------------|
| 0–2 | 1 | Light | economy |
| 3–5 | 2 | Standard | standard |
| 6+ | 3 | Deep | advanced |

The **Model Class** column is an abstract effort lever: tier scales the model class the same way it scales researcher depth (`quick`/`deep`) and Phase 4 specialist depth — `economy` for cheap mechanical changes, `standard` for routine multi-file work, `advanced` for high-blast-radius reasoning. It is a hint resolved per-adapter against that adapter's model map (`src/models/resolve.ts::resolveAgentModel`, `models.default`), not a literal model id — adapters with no model-routing surface ignore it. Model class is a first-order effort lever alongside depth (`hatch3r-agent-orchestration` -> Tier-to-Phase-4 specialist depth mapping); the per-agent class floor and the per-spawn allocation matrix (`max(agent floor, task-tier class)`) live in `rules/hatch3r-model-allocation.md`.

...
- **UI/UX floor link**: the `fetchpriority="high"` LCP hint and deferred non-critical JS above are this rule's half of the generated-UI Core Web Vitals floor; the layout-stability half (`aspect-ratio`, explicit `width`/`height`, INP task-splitting) lives in `rules/hatch3r-ux-states-and-flows.md`, and `skills/hatch3r-ui-ux-verify` reaches both halves in one gate.

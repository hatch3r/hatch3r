...
This rule applies to EVERY context without exception: board-pickup (epic, sub-issue, standalone, batch), workflow command (full/quick), plain chat, issue references, and natural-language requests. Every task MUST follow the four-phase pipeline — **Phase 1 Research** (`hatch3r-researcher`), **Phase 2 Implement** (`hatch3r-implementer`), **Phase 3 Review Loop** (`hatch3r-reviewer` + `hatch3r-fixer`), **Phase 4 Final Quality** (parallel specialists) — per Mandatory Delegation Directives below; never implement code inline without sub-agents. Pipeline **intensity** is tier-scaled, not static: the triage tier prunes phase depth per `agents/shared/triage-vocabulary.md` → Pipeline pruning per tier and the Phase Skip Criteria below (Tier 1 collapses research+plan into one pass, runs a single implementer, and caps the review loop at 1 iteration); pruning is tier-derived, never token-cost-derived, and the four-phase structure plus the delegation mandate bind at every tier.
...
**Tier-to-Phase-4 specialist depth mapping.** Deep-context tier drives Phase 1 researcher depth; the same **effective** tier (baseline `deepContextTier`, or a mid-run upgrade resolved via `resolveEffectiveTier`) drives Phase 4 specialist depth so quality coverage scales with task risk: Tier 1 → run only the always-mode floor (`hatch3r-security` + `hatch3r-testability`) at `quick` depth (the floor itself is skippable solely under the four-criteria Tier-1 relaxation in Phase Skip Criteria) — UI/perf/maintainability/etc. specialists (mandatory-on-match included) are skipped per Phase Skip Criteria. Tier 2 → always-mode floor at `standard` depth + every triggered conditional specialist at `quick` depth, and a triggered mandatory-on-match specialist (`hatch3r-ui` CQ1 / `hatch3r-ux` CQ2) MUST spawn as its own dedicated sub-agent instance (never merged into one spawn) — skipping a triggered one at Tier 2/3 is a gate failure. Tier 3 → every applicable specialist at `deep` depth (same mandatory-on-match mandate) — full WCAG AA / OWASP ASI / CWV / mandate-map sweep with N=3 sampling on always-mode specialists when `floor:security` items are touched (SPEC, not yet enforced — `varianceTracker.ts` is unimplemented and `--variance-runs` is unregistered; until it ships, every run is single-pass per `agents/shared/quality-charter.md` -> Non-Determinism Budget). The depth signal rides on the specialist prompt as the explicit field `depth: quick | standard | deep`; specialists read it via the shared `agents/shared/quality-specialist-frame.md`. Tier also drives **model class** as a first-order effort lever (Tier 1 → economy, Tier 2 → standard, Tier 3 → advanced), resolved per-adapter against `models.default` / `src/models/resolve.ts` and ignored where an adapter has no model-routing surface — see `hatch3r-deep-context` -> Tier Assignment; per-agent floors + the per-spawn allocation matrix: `rules/hatch3r-model-allocation.md`.
...
2. Critical/Warning findings: spawn `hatch3r-fixer` with full reviewer output, then re-review. Repeat until 0 open Critical + 0 open Warning or the iteration cap. **Unified cap scheme (stated once here; every consumer cites it):** the operative bound is the loop-class cap from `REVIEW_LOOP_CLASS_CAPS` in `src/pipeline/reviewLoop.ts` — 3 for code-diff loops, 4 for spec/issue-text loops — under the protocol ceiling `DEFAULT_MAX_REVIEW_ITERATIONS = 4` (same module); configured overrides clamp to `HARD_MAX_REVIEW_ITERATIONS = 10`, and Tier 1 collapses to 1 iteration per Universal Applicability. The oscillation detector is decoupled and fires at `history.length >= 3` independent of the cap (kept in sync by `src/__tests__/pipeline/reviewLoop.test.ts`, CI-enforced). Before each fixer dispatch, append the W1 write-ahead rows; after each re-review, append the W2 disposition rows (`rules/hatch3r-findings-ledger.md` → Write Points).
   - **Re-run honor-rule (anti-self-approval).** The fixer's `Reviewer re-run required` boolean is advisory; the orchestrator derives the authoritative value `reRunRequired = (fixer Files changed list is non-empty)`. When `true`, another `hatch3r-reviewer` pass is MANDATORY before the loop may exit clean — a fixer `Status: SUCCESS` with a non-empty `Files changed` can never self-approve to a clean exit. A `Reviewer re-run required: false` printed alongside a non-empty `Files changed` is overridden to `true` and noted as a self-declared protocol violation. The `Files changed` list is SSOT-bound: it is attested by the same fixer `delegation_proof_id` the orchestrator quotes in its End-of-Turn Delegation Attestation, so the signal cannot be forged without spawning the fixer.
   - **Delta re-review scope (from iteration 2).** Iteration 1 reviews the full change set with the full checklist. From iteration 2, the re-review scope is (a) the hunks changed since the previously reviewed state — the fixer's `Files changed` diff — plus (b) findings the orchestrator marked `verify-fix`; a full re-review runs only on iteration 1 or on explicit user/orchestrator request. The orchestrator passes the prior-iteration findings table AND the delta diff in every re-review prompt (`agents/hatch3r-reviewer.md` → Delta Re-Review Scope).
   - **Finding fingerprints + severity gating (kills re-litigation).** Identify findings by fingerprint — file + normalized hunk + finding class; a fingerprint already dispositioned (fixed / accepted / won't-fix) is never re-raised on unchanged code. From iteration 2, only Critical/Warning findings (CRITICAL/HIGH on the Severity Scale below) re-open the loop; Suggestion-class findings are recorded once and carried forward without re-litigation.
3. **Confirmation pass** after clean review: lightweight re-review checking only (a) no new test failures vs Phase 2 baseline, (b) no type errors introduced, (c) acceptance criteria still met. Does not re-run the full checklist.
   - **Runtime calibration second pass (orchestrator-owned).** At this would-be-clean exit, the orchestrator — not the stateless reviewer — evaluates the `rules/hatch3r-reviewer-calibration.md` trigger: read the cross-run `consecutive_clean_pass_count` from `.hatch3r/calibration-state.json`, increment it for this clean run, and fire a second-pass review (different model class, else same-class re-roll) when the post-increment count is a multiple of `N` (default 5) OR — for a high-risk diff (`floor:security` / auth / CQ3-security-dispatch files) — on the first clean PASS. A divergent second pass reverts the exit to step 2 (`REQUEST CHANGES`) and resets the count to 0 — with no iteration budget remaining under the class cap, exit UNRESOLVED and escalate both verdicts instead (never iterate past the cap); persist the count (atomic write via `src/merge/safeWrite.ts`) and append the calibration-log record. A REQUEST CHANGES or DESIGN_OBJECTION at any iteration also resets the persisted count to 0.
4. **Exit states — never silently continue.** Every loop exit is exactly one of: **PASS** (0 open Critical + 0 open Warning; proceed through step 3's confirmation pass), **PASS-WITH-NOTES** (0 open Critical/Warning with open Suggestions carried forward — surfaced to the user, never auto-fixed or re-litigated), or **UNRESOLVED** at the class cap: surface to user with a structured summary (iteration count, remaining Critical findings with file:line, remaining Warnings, fix-manually-vs-accept-risk recommendation) — a cap-out escalates; it never spawns a further reviewer or fixer pass (`agents/shared/confidence-gate.md` Decision step 4). Never present raw reviewer output unsummarized. Reconcile the ledger to the run-exit invariant (W3) before surfacing; the structured summary lists the open `finding_id`s.
5. **Review gate confidence signal:** on a clean verdict, record the iteration count in `PipelineContext.reviewResult.iterations`. Clean-on-first-pass signals higher confidence than clean-after-multiple-iterations; Phase 4 and the orchestrator factor this into risk assessment. **Loop-exit telemetry:** at every review-loop exit (clean, max-iterations, or abort), append one JSONL exit record to `.hatch3r/review-loop-metrics.jsonl` — schema and canonical writer: `src/pipeline/reviewLoop.ts::appendReviewLoopTelemetry` (recordedAt, loopClass, maxIterations, iterationCount, terminationReason, verdictByIteration, converged, optional durationMs, source). A failed telemetry write is a warning, never a pipeline blocker; the ledger is the runtime data source that promotes the review-loop CALIBRATION to `basis: "measured"` at 30 samples.

**Phase 4 — Final Quality** (after review loop is clean):

Launch Phase 4 specialists in parallel, bounded by an orchestrator-honored fan-out width `max_phase4_parallel` (default `8` — the empirical concurrency ceiling for a single coordinating orchestrator (Anthropic multi-agent research: 3-5 typical, ~8 upper bound), so a maximal Tier 3 change triggering all 13 specialists fans out in at most 2 batches). This bound is LLM-honored orchestrator guidance, not a code-enforced cap: the host Task tool is the actual dispatcher and applies no platform fan-out limit, so no hatch3r module reads an env var or clamps the count — the orchestrator self-limits per this prose. The bound exists for upstream provider rate-limit headroom (RPM/TPM) — a true dependency edge — NOT per-orchestrator context cost; token cost never serializes independent work (P8 dominates P7). A non-rate-limited orchestrator MAY raise the width up to the full applicable-specialist set. **Rate-limit back-off (orchestrator-LLM guidance):** when the orchestrator observes ≥3 consecutive rate-limit-class transient failures, reduce the active fan-out width by 1 for the next batch and record the back-off in the Iteration Summary; never silently cap a healthy run. When applicable specialists exceed the bound, batch by severity-descending priority `CRITICAL → HIGH → MEDIUM → LOW` (severity is the worst-case finding class the specialist surfaces: always-on testability (CQ5) / security (CQ3) → CRITICAL, mandatory-on-match CQ1/CQ2 (ui/ux) + conditional CQ4/CQ7 (reliability/performance) → HIGH, docs/lint → MEDIUM, low-impact → LOW); within a bucket, dispatch in trigger-table order. Each batch runs to completion before the next starts; the validation pass runs once after the final batch. The applicable specialists and their trigger conditions are listed in the Phase 4 Specialist Trigger Table below. The width bound and the rate-limit back-off never drop a triggered mandatory-on-match specialist — `hatch3r-ui`/`hatch3r-ux` dispatch as dedicated instances in the first HIGH batch.

**Specialist Prompt Enrichment:** When spawning Phase 4 specialists, include the Phase 2 `filesChanged` list (focus on affected code), the Phase 3 review verdict summary (avoid re-flagging reviewed issues), and `researchFindings.blastRadius` (assess downstream impact) — scoped per `rules/hatch3r-context-budget.md`.

**Runtime trigger evaluation:** the orchestrator harness calls `shouldTriggerSpecialist(specialist, changedFiles, projectType)` from `src/pipeline/pipelineContext.ts` to evaluate whether each specialist applies to the current change set. The function returns `{ triggered: boolean, reasons: string[], mandatory?: boolean }` and consumes the same `SPECIALIST_TRIGGER_TABLE` that the prose table below mirrors. Treat the prose as a quick reference; treat the TS predicate as the authoritative gate. `npm run validate:specialist-roster` enforces parity. The orchestrator treats `mandatory: true` as non-skippable at Tier 2/3.

**Phase 4 Specialist Trigger Table:**

| Specialist | Mode | Trigger Conditions |
|-----------|------|--------------------|
| `hatch3r-docs-writer` | Evaluate | Public API, architecture, or UX changes; unconditional when the diff changes user-observable behavior covered by `docs/specs/**` (`rules/hatch3r-spec-currency.md`) |
...
**Sub-agent-failure handling (shared clause; all commands cite this — never inline).** When any spawned implementer/fixer/specialist sub-agent FAILS or returns no usable output: (1) retry once with the same prompt; (2) if the retry fails, re-spawn `hatch3r-fixer` (Phase 3) with the failure reason + partial output as failure context — `hatch3r-fixer` is the code-mutation path, so the work stays delegated; (3) if the re-spawn also fails, emit `BLOCKED_OTHER` with a one-sentence reason and ASK the user (fix-manually vs adjust-scope vs accept-risk). Flush in-flight ledger rows per W4 — before the retry and again before the BLOCKED_OTHER ASK (`rules/hatch3r-findings-ledger.md`). The orchestrator MUST NOT fall back to inline implementation — that is the bypass mode (see Mandatory Delegation Directive). The sole exception is `hatch3r-quick-change`, whose Tier-1 carve-out permits inline retry per its declared scope. **Quality-gate-failure escalation (sibling shared clause; all delivery commands cite this).** When project quality gates (lint/typecheck/test) still fail after the bounded 2 retries, the default across delivery commands is a single ASK — commit-partial / keep-trying / abort. A command whose flow justifies continue-with-PARTIAL instead (e.g. `hatch3r-pr-resolve`, resolving pre-existing PR comments) MUST declare that exception inline with its rationale AND emit an explicit pre-push warning before pushing gate-failing code; a command that loops on the gate MUST bound the loop. Undeclared divergence across the delivery commands — silent ASK vs continue-and-push vs unbounded re-run on the same event — is the contradiction this shared home closes.
...
|-------|--------------|--------------------------------------|
| **Phase 1 (Research)** | Trivial single-line edit (typo, comment, single-value config); Tier 1 single-file change with no cross-module impact; Research already cached in PipelineContext | Affected files identified (even via quick scan); existing tests noted; `plan_gate: true` commands: persisted plan artifact approved (In-Session Plan Gate) |
...
| **Phase 4 (Quality)** | Review loop unresolved AND user chose manual resolution; documentation-only; all trivial + quality checks pass (quick-change only) | testability (CQ5) + security (CQ3) required for every code change except under the Tier-1 relaxation below; quality checks must pass |

**Tier-1 relaxation (criteria-based).** Tier-1 (light) runs may skip the CQ5 testability + CQ3 security specialist waves only when ALL of: (1) diff ≤50 changed LOC; (2) no new dependencies; (3) no files under security-adjacent paths (auth, crypto, secrets, input parsing/validation, permissions); (4) change class is docs-only, config-only, copy/comment-only, or single-function fix with existing test coverage. Any unmet criterion pins CQ5+CQ3 back on. The relaxation is tier-derived (task decomposition), never token-cost-derived; citing token cost to skip a wave remains a P8 violation. See `src/pipeline/pipelineContext.ts` for the programmatic `PHASE_SKIP_CRITERIA` constant and the mirrored relaxation criteria.

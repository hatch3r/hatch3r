...
2. Critical/Warning findings: spawn `hatch3r-fixer` with full reviewer output, then re-review. Repeat until 0 Critical + 0 Warning, or max 4 iterations (matches `DEFAULT_MAX_REVIEW_ITERATIONS` in `src/pipeline/reviewLoop.ts`, cap set from convergence economics per Finding D7-16 — the oscillation detector is decoupled and fires at `history.length >= 3` independent of the cap; kept in sync by `src/__tests__/pipeline/reviewLoop.test.ts`, CI-enforced). Before each fixer dispatch, append the W1 write-ahead rows; after each re-review, append the W2 disposition rows (`rules/hatch3r-findings-ledger.md` → Write Points).

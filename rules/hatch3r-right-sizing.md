...
1. **Adapter header** — generated artifacts carry the tier near their top. Claude (`CLAUDE.md`) and Copilot: a visible blockquote `> hatch3r: right-size to maturity=<tier>…` (Claude Code strips block-level HTML comments from `CLAUDE.md` before reading it, so the directive ships visible, not commented). Cursor: `<!-- hatch3r: right-size to maturity=<tier>… -->` in each rule body. This is the fastest and most authoritative signal at agent runtime.

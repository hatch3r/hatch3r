...
type: rule
description: "Recap-contract iteration summary — every orchestrator command and meaningful skill run closes with a 1-2 line recap (status, outcome, files/sub-agents/gates/cost-delta telemetry) plus exception lines that fire on non-default outcomes with one always-emitted exception — the `Not done:` line appears on every recap (`Not done: none — full scope completed` when empty) — and, at Tier >= 2, a Completion Ledger (done/deferred/blocked per scope item with N-of-M totals, cumulative across epic/batch iterations) whose <100% state fires the single bundled Disposition Gate ASK (continue now / keep deferred / queue-or-handoff / stop); a Remaining Work terminal block closes the run when scope items remain, each item carrying a `disposition:` suffix; silence asserts the default for every other line."
...
Every orchestrator command (`commands/hatch3r-*.md` with `orchestrator: true`) AND every meaningful skill run (`/h4tcher-*` or `/hatch3r-*` that mutates state) MUST close with the recap-contract Iteration Summary as the final user-facing output — with at most two sanctioned trailers after it: the Plan-Execution Handoff block (see §Plan-Execution Handoff below), then the Remaining Work block (§Remaining Work (terminal block)), which when it fires is the run's very last output — with the Completion Ledger (§Completion Ledger) and its Disposition Gate ASK (§Disposition Gate) as the one sanctioned interposition sequence before that final block. The block opens with the literal heading `## Iteration Summary` — the stable extraction anchor for downstream consumers. Governance anchor: CONSTITUTION §6 Decision 41 (2.8.6; Replaces: 38; lineage 28 → 37 → 38 — recap contract → always-explicit completion → disposition gate, extended with the Tier ≥ 2 Completion Ledger).
...
|---|---|---|---|---|
| Not done | `Not done: <item> [— deferred: <why> \| blocked: <blocker> \| unverified: <what>]; …` or `Not done: none — full scope completed` | always (the empty form is the positive assertion of full completion) | charter honesty; handoff Work Remaining | never — always emitted |
...
- **Work Remaining** ← the Completion Ledger's non-done item lines (state + reason per item; a full-completion `(<N>/<N>)` ledger ⇒ no remaining work); Tier 1 runs and legacy transcripts without a ledger fall back to the `Not done:` line verbatim (`none — full scope completed` ⇒ none; a missing line ⇒ `None — full scope completed`)
...
A run fails the gate when the recap is missing, when any registry row's firing condition holds with no corresponding exception line, or when prose is substituted for the recap grammar. A `plan_handoff: true` run that ends at the plan seam (stop/defer, `--auto`, `--review-only`, non-interactive — §Plan-Execution Handoff) and closes without the Plan-Execution Handoff block fails the gate identically; a run that continued through the Execute-Now Continuation is exempt — emitting the block there is itself the violation. A recap missing the `Not done:` line fails the gate — the line is always-emitted, never a silent default. A run whose `Not done:` line carries ≥1 item but that does not close with the `## Remaining Work` terminal block (§Remaining Work (terminal block)) fails the gate. A `## Remaining Work` item line missing its `disposition:` suffix fails the gate, as does a disposition-eligible run (a Completion Ledger below 100% — ≥1 deferred or blocked item, unresolved follow-up, or punted human decision) that emitted neither the bundled disposition ASK nor the `Default applied: kept deferred (non-interactive)` line (§Disposition Gate). A Tier ≥ 2 orchestrated flow that terminates without the Completion Ledger fails the gate (§Completion Ledger), as does a ledger whose item set disagrees with the `Not done:` line. Registry-row checking is an explicit pre-status step: before declaring status, the orchestrator walks the table above and emits each line whose firing condition holds — the omission is caught before SUCCESS is declared.
...
At Tier ≥ 2, immediately before the Disposition Gate / Remaining Work sequence — after the recap, its exception lines, and the Plan-Execution Handoff block when it fires — the orchestrator MUST emit the run's completion inventory:
...
Line 1 totals the run's scope items (a + b + c = N); one indented line follows per non-done item naming its state (`deferred` or `blocked`) and reason. At 100% completion (a = N) the ledger is the single totals line and NO disposition ASK fires — the totals line is the positive assertion of full completion. Scope items are the same units the `Not done:` line tracks; the ledger adds the `blocked` class and the N-of-M totals, and the two surfaces MUST agree (an item on one and not the other fails the §Validation Gate). Tier 1 runs skip the ledger — the always-emitted `Not done:` line carries completion state alone.

**Cumulative form (epic/batch flows):** flows iterating over sub-issues or dependency levels maintain ONE cumulative ledger across iterations and re-emit the updated totals line at each iteration boundary — `sub-issues: done <a> · deferred <b> · blocked <c> (<a>/<N>)` — so the 100%-completion view never washes out mid-run; the final iteration's ledger is the run-level ledger.

## Disposition Gate

Fires when the Completion Ledger shows <100% completion — ≥1 deferred item, blocked item, unresolved follow-up, or punted human decision (widened 2026-08-03 from the deferred-only trigger). A 100% ledger ⇒ the gate passes silently — no ASK, no block (an empty ASK is itself a violation). Purpose: open items otherwise accumulate silently across runs, letting a run read as done while decisions pile up unrecorded — the gate converts each item into one recorded human decision before the run may close.
...
- The question body opens with the ledger totals line and lists every non-done item, numbered. When >4 items, group them under ≤4 labels (by file, domain, or theme) and number the groups so the ASK stays within one screen.
- Options (fixed): `1. Continue now (follow up in this session)` · `2. Keep deferred (default)` · `3. Queue as a new workflow or prepare handoff (/hatch3r-handoff prepare)` · `4. Stop here`. The reply may split by item/group number (e.g., "1 for items 1 and 3, 2 for the rest"); every item not explicitly assigned takes the default.
- One ASK total per run — never one per item, and never a second ASK after a follow-up pass.
- Non-interactive/headless runs skip the ASK, apply option 2 to every item, and emit the registered exception line `Default applied: kept deferred (non-interactive)`.
- Option 1 items re-enter execution BEFORE the summary re-emits: run the follow-up work, then re-emit the Iteration Summary and ledger — completed items drop off; items still open carry `disposition: followed-up` in the re-emitted Remaining Work block, with no second ASK.
- Option 3 items record `disposition: queued:<ref>` where `<ref>` names the created artifact (plan file, issue id, or board item), or `disposition: handoff:<ref>` where `<ref>` names the handoff document produced by `/hatch3r-handoff prepare`.
- Option 4 closes the run immediately: remaining items record `disposition: stopped` — an explicit user decision to end with the item open; epic/batch flows do not advance to further iterations.

Disposition suffix: every item line in the `## Remaining Work` block carries a mandatory `disposition: kept | followed-up | queued:<ref> | handoff:<ref> | stopped` suffix. An item line without one is a gate failure (§Validation Gate) — no open item may appear without a recorded human decision (or the recorded non-interactive default).

## Remaining Work (terminal block)

Fires when the `Not done:` line carries ≥1 item (not the `none — full scope completed` form) — at Tier ≥ 2, equivalently, when the Completion Ledger shows <100% (Decision 37/41). Position: the VERY LAST output of the run — after the recap, its exception lines, and the Plan-Execution Handoff block when both fire. The Completion Ledger and its Disposition Gate ASK (§Completion Ledger, §Disposition Gate) are the only elements permitted between the run-closing sequence — End-of-Turn Delegation Attestation (`rules/hatch3r-agent-orchestration.md`), recap, trailers — and this final block; the block itself stays last.

Format: the heading `## Remaining Work`, then the `Not done:` line copied verbatim with each item gaining the mandatory `disposition: kept | followed-up | queued:<ref> | handoff:<ref> | stopped` suffix (§Disposition Gate), then the `Blockers:` and `Open findings:` lines copied verbatim only when those recap lines fired — the disposition suffix is the block's one bookkeeping addition beyond re-surfaced recap data.
...
## Pillar Service
- P1 — the always-emitted `Not done:` line, the Tier ≥ 2 N-of-M Completion Ledger, and the Remaining Work terminal block make incompleteness un-missable at run end (actionability)
- P8 — the disposition gate routes every <100% ledger through one bundled B1 ASK — continue / keep deferred / queue-or-handoff / stop (`agents/shared/user-question-protocol.md`) — so run-end deferral is a recorded human decision, never silent accumulation

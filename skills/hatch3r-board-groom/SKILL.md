...
Run directly for backlog refinement, or dispatched from `skills/hatch3r-cleanup/SKILL.md` (Step 2) scoped to stale-item archival (Step 4e) after that skill's consolidated cleanup ASK — this skill's own per-issue ASK checkpoints still apply on dispatch.

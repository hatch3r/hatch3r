...
| **iteration-summary** | 9-section iteration summary emitted by every orchestrator command and meaningful skill run -- status, outcome, fan-out + cost, gates, pillar impact, deferred-items disposition gate. |
| **communication-style** | Operator-facing register dial -- plain (default: outcome first, <=3 headline facts before detail, first-use glosses, <=25-word average sentences) or technical, keyed off the manifest communicationStyle field; machine-parseable grammars exempt. |

...
| **model-allocation** | Model-class allocation per sub-agent spawn -- the economy/default/strongest vocabulary, static per-agent class floors, the max(agent floor, effective-tier class) matrix, an explicit per-spawn model pass, and floor pinning for verdict-class specialists. |

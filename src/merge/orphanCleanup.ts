 ...
 *
 * D10-SA10.6-01 (release/2.8.6): `SKILL.md` is additionally recognized when
 * its PARENT directory basename carries the hatch3r marker — skill emissions
 * land at `<adapter-root>/skills/hatch3r-<id>/SKILL.md`, so the managed
 * marker lives on the directory, not the file. Without this, a deselected
 * skill's SKILL.md survived every sweep as a permanent `! orphan` row
 * (`not-managed-basename`). A `SKILL.md` under a NON-hatch3r parent (a
 * user-authored skill directory) is still refused.
 */
function isManagedOutputBasename(fileName: string, parentDirName?: string): boolean {
  if (fileName.startsWith(HATCH3R_PREFIX) || NN_HATCH3R_PREFIX_RE.test(fileName)) return true;
  if (fileName === "SKILL.md" && parentDirName !== undefined) {
    return parentDirName.startsWith(HATCH3R_PREFIX) || NN_HATCH3R_PREFIX_RE.test(parentDirName);
  }
  return false;
}
...
async function fileIsUserWrapped(
  ...
  try {
    ...
    if (!hasManagedBlock(content, absPath)) {
      // No block at all => treat as fully managed. Fully user files are
      // protected by CANDIDACY — only manifest-recorded prior emissions
      // reach this check (diffOrphanCandidates over managedFilesByAdapter) —
      // not by the basename filter, which passes hatch3r-marked names
      // including SKILL.md under a hatch3r-* parent (review fix F4).
      return { wrapped: false };
    }
    // Block present. If anything outside the block is user-authored, we must
    // not touch the file. The split helpers handle any known marker variant
    // (issue #76: YAML `#` markers in `.github/workflows/*.yml`), so we don't
    // hard-code HTML markers here.
    ...
    //
    // D10-SA10.6-01 (release/2.8.6): the BEFORE-block slice is judged with
    // `isHealableManagedPrefix` — the same generated-vs-user discriminator the
    // sync-side stub heal uses — instead of "any non-whitespace bytes". The
    // with-frontmatter emission paths (`processCommandsWithFm`,
    // `processSkillsWithFmCliFiltered`, release/2.6.0) put a GENERATED byte-0
    // picker stub above the BEGIN marker, so every skill/command orphan would
    // otherwise be permanently vetoed as user-wrapped by content hatch3r
    // itself wrote. A prefix that is anything other than whitespace or exactly
    // one frontmatter block (prose, unterminated fence, extra content) is
    // still user content; ANY non-whitespace suffix after END remains an
    // unconditional veto.
    ...
    return { wrapped: prefixIsUser || suffixIsUser };
  } catch (err) {
...
export async function sweepOrphansForAdapter(
  ...
  for (const relPath of candidates) {
    ...
    // Filter 2: basename must be hatch3r-* or NN-hatch3r-* — or SKILL.md
    // under a hatch3r-* parent directory (the skill-emission layout;
    // D10-SA10.6-01). A path inside an adapter root but without any hatch3r
    // marker is one we never emitted; refuse the unlink. release/2.8.5: a
    // caller-supplied `trustedExactPaths` entry (manifest-recorded output of a
    // deselected adapter, swept under explicit consent) bypasses ONLY this
    // filter — containment already passed above and the user-wrapped veto
    // below still protects files carrying user content outside the managed
    // markers.
    const name = basename(relPath);
    const parentDirName = basename(dirname(relPath));
    const trusted = options?.trustedExactPaths?.has(relPath.replace(/\\/g, "/")) === true;
    if (!isManagedOutputBasename(name, parentDirName) && !trusted) {
    ...
    try {
      ...
      // F6 (review, D10-SA10.6-01): a swept skill's SKILL.md leaves its
      // hatch3r-owned directory empty — remove the directory best-effort so
      // the deselected skill folder does not linger. Guarded to
      // hatch3r-marked parent basenames (the same recognition filter 2
      // applied); `rmdir` is non-recursive, so a directory still holding
      // user files fails ENOTEMPTY and survives. Every failure is non-fatal:
      // the unlink already succeeded and the entry stays `unlinked`.
      if (name === "SKILL.md" && isManagedOutputBasename(name, parentDirName)) {
        try {
          ...
          verbose(
            code === "ENOTEMPTY"
              ? `orphanCleanup: skill directory ${dirname(absPath)} left in place — contains other files`
              : `orphanCleanup: rmdir(${dirname(absPath)}) skipped — ${message}`,
          );
        }
      }

...
describe("isHealableManagedPrefix", () => {
  ...
  it("accepts the identical stub re-encoded with CRLF line endings (git autocrlf working tree)", () => {
    // A bare `\r` is a line-ending artifact, not padding — it cannot smuggle
    // content, and refusing it would veto every sweep/heal on a Windows
    // autocrlf checkout of the same generated bytes.
    expect(isHealableManagedPrefix('---\r\nname: x\r\n---\r\n\r\n')).toBe(true);
  });
  ...
  // sec-2.8.6-b2-p4: the predicate also gates orphan-sweep deletions, so
  // acceptance is pinned to hatch3r's emitted stub byte shape — fences are
  // EXACTLY `---` at column 0 and the opening fence is the first line.
  // hatch3r never emits padded/indented fences or a blank-line-prefixed stub,
  // so every looser look-alike is user content.
  it("rejects a blank line before the opening fence (tightened: stub starts at byte 0)", () => {
    expect(isHealableManagedPrefix('\n---\nname: x\n---\n  \n')).toBe(false);
  });

  it("rejects padded or indented fence lines (tightened: exact column-0 `---` only)", () => {
    expect(isHealableManagedPrefix('  ---\nname: x\n---\n')).toBe(false);
    expect(isHealableManagedPrefix('---  \nname: x\n---\n')).toBe(false);
    expect(isHealableManagedPrefix('---\nname: x\n  ---  \n')).toBe(false);
    expect(isHealableManagedPrefix('---\nname: x\n--- \n')).toBe(false);
  ...
  it("column-0 exact fences remain healable after the tightening", () => {
    expect(isHealableManagedPrefix('---\nname: hatch3r-old-command\ndescription: "Old command"\n---\n\n')).toBe(true);
  });

  it("interior lines between the fences are opaque — padded --- look-alikes are fence interior (review-2.8.6-b2-r3 N1)", () => {
    // Chosen semantics, pinned: the predicate inspects only the fence lines
    // themselves. `  --- ` at line 3 is not an exact column-0 fence, so it is
    // interior content; the exact `---` at line 5 closes the block and the
    // prefix is healable. This byte class is a hand-edited stub, and the sync
    // heal clobbers the whole stub with the freshly generated one on the next
    // sync anyway — treating it as healable loses no user-owned content.
    expect(isHealableManagedPrefix('---\na: 1\n  --- \nnote\n---\n')).toBe(true);
  });
});
